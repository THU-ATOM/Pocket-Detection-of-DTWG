...
class DockingTestCallback(Callback):
    ...
    def on_test_epoch_end(
        ...
        good_case_dir = os.path.join(path, 'good_cases')
        os.makedirs(good_case_dir, exist_ok=True)
        print(f'good cases dumped to {good_case_dir}')

        try:
            output_dir=pl_module.cfg.output_dir
            if not os.path.exists(output_dir):
                os.makedirs(output_dir, exist_ok=True)
        except:
            output_dir=None
        out_metrics = self.metric.evaluate(results, bad_case_dir, good_case_dir,output_dir)
        torch.save(results, os.path.join(path, f'vina_docked.pt'))
        out_metrics.update(recon_dict)
