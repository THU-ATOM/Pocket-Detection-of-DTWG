...
class CondMolGenMetric(object):
    ...
    def compute_chem_results(self, generated: list[dict]):
        ...
        for item in tqdm(generated, total=len(generated), desc="Chem eval"):
            ...
            try:
                # docking
                if self.docking_config is not None and self.docking_config.mode is not None:
    ...
    def evaluate(self, generated: list[dict], bad_case_dir: str = None, good_case_dir: str = None, output_dir: str = None):
        ...
        for idx, res in enumerate(results):
            ...
            if output_dir is not None:
                good_case_dir=None
                bad_case_dir=None
                if ligand_filename not in name_cnt:
                    name_cnt[ligand_filename] = 0
                with Chem.SDWriter(os.path.join(output_dir, f'{ligand_filename.split(".")[0]}_{name_cnt[ligand_filename]}.sdf')) as w:
                    w.write(res['mol'])
                name_cnt[ligand_filename] += 1
            ...
            try:
                ...
                if vina_score > 0 or vina_min > 0:
                    ...
                    # save good case
                    if good_case_dir is not None:
                        mol = res['mol']
                        with Chem.SDWriter(os.path.join(good_case_dir, f'{ligand_filename.split("/")[0]}_{idx}.sdf')) as w:
                            w.write(mol)
